...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['mp3File']) && $_FILES['mp3File']['error'] === UPLOAD_ERR_OK) {
        ...
        // echo '<script>alert("Berhasil konversi file")</script>';
    } else {
        echo '<script>alert("Terjadi kesalahan saat mengunggah file MP3.")</script>';
        header("Location: mp3toWav.php");
        exit();
    }
} else {
...
<body>
    ...
    <div class="content-row text-center">
        <?php
        echo "<a class='column btn' href='$wavFilePath' download='output.wav'><h2>Download File</h2></a>";
        ?>
    </div>
    <div class="content-row text-center">
        <a class="text-hitam" href="mp3toWav.php">Convert Other File</a>
    </div>
