...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle file upload
    if (isset($_FILES['wavFile']) && $_FILES['wavFile']['error'] === UPLOAD_ERR_OK) {
        // Ubah nama file menjadi 'uploadfile'
        $wavFileName = 'uploadfile.wav';
        $wavFilePath = $uploadDir . $wavFileName;

        move_uploaded_file($_FILES['wavFile']['tmp_name'], $wavFilePath);

        // Buat nama file output Wav
        $mp3FileName = pathinfo($wavFileName, PATHINFO_FILENAME) . '.mp3';
        $mp3FilePath = $uploadDir . $mp3FileName;

        // Gunakan perintah ffmpeg untuk mengkonversi MP3 ke Wav
        $command = "\"$ffmpegPath\" -i $wavFilePath -b:a 192k $mp3FilePath";
        shell_exec($command);
        ...
        // echo "Konversi selesai. <a href='$mp3FilePath' download>Unduh Wav</a>";
    } else {
        echo '<script>alert("Terjadi kesalahan saat mengunggah file Wav.")</script>';
        header("Location: wavtoMp3.php");
    }
} else {
...
<body>
    ...
    <div class="content-row text-center">
        <a class="text-hitam" href="wavtoMp3.php">Convert Other File</a>
    </div>
